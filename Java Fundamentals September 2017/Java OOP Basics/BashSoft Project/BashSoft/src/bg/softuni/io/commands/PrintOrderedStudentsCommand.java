...
public class PrintOrderedStudentsCommand extends Command {
    public PrintOrderedStudentsCommand(String input, String[] data, StudentsRepository repository, Tester tester, IOManager inputOutputManager, DownloadManager downloadManager) {
    ...
    private void tryParseParametersForOrder(
        ...
        if (takeQuantity.equals("all")) {
            super.getRepository().orderAndTake(courseName, orderType);
            return;
        }

        try {
            int studentsToTake = Integer.parseInt(takeQuantity);
            super.getRepository().orderAndTake(courseName, orderType, studentsToTake);
